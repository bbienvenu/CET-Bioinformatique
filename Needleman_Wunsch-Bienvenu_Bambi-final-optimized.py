...
# Projet : détermination de tous les alignements optimaux issus de l’algorithme de Needleman et Wunsch (version optimisée)
...
def calcul_alignements(seq1, seq2, match, mismatch, gap, scores, i=None, j=None, graphe=None, visited_nodes=list(), visited_nodes_values=list()):
    '''
    ...
    if i == 0 and j != 0:
        ...
        # si le noeud n'a pas encore été traité : on le crée, le traite puis le rajoute à la liste des noeuds traités
        if (i,j) not in visited_nodes_values:
            graphe.gauche = Tree((i,j-1))
            calcul_alignements(seq1, seq2, match, mismatch, gap, scores, i, j-1, graphe.gauche)
        
        # si le noeud a déjà été traité, on l'affecte au fils correspondant
        else:
            index = visited_nodes_values.index((i, j))
            graphe.gauche = visited_nodes[index]
    ...
    if j == 0 and i != 0:
        ...
        # si le noeud a déjà été traité, on l'affecte au fils correspondant
            ...
            index = visited_nodes_values.index((i, j))
            graphe.haut = visited_nodes[index]
    ...
    if scores[i][j] == scores[i][j-1] + gap:
        ...
        # si le noeud n'a pas encore été traité : on le crée, le traite puis le rajoute à la liste des noeuds traités
        if (i, j-1) not in visited_nodes_values:
            graphe.gauche = Tree((i,j-1))
            calcul_alignements(seq1, seq2, match, mismatch, gap, scores, i, j-1, graphe.gauche)
            
        # si le noeud a déjà été traité, on l'affecte au fils correspondant
        else:
            index = visited_nodes_values.index((i, j-1))
            graphe.gauche = visited_nodes[index]
    ...
    if scores[i][j] == scores[i-1][j-1] + match_ou_mismatch(seq1[i-1],
        ...
        # si le noeud n'a pas encore été traité : on le crée, le traite puis le rajoute à la liste des noeuds traités
        if (i-1, j-1) not in visited_nodes_values:
            graphe.diagonale = Tree((i-1,j-1))
            calcul_alignements(seq1, seq2, match, mismatch, gap, scores, i-1, j-1, graphe.diagonale)
        
        # si le noeud a déjà été traité, on l'affecte au fils correspondant
        else:
            index = visited_nodes_values.index((i-1, j-1))
            graphe.diagonale = visited_nodes[index]

    # gap dans la séquence 2
    if scores[i][j] == scores[i-1][j] + gap:
        
        # si le noeud n'a pas encore été traité : on le crée, le traite puis le rajoute à la liste des noeuds traités
        if (i-1, j) not in visited_nodes_values:
            graphe.haut = Tree((i-1,j))
            calcul_alignements(seq1, seq2, match, mismatch, gap, scores, i-1, j, graphe.haut)
        
        # si le noeud a déjà été traité, on l'affecte au fils correspondant
        else:
            index = visited_nodes_values.index((i-1, j))
            graphe.haut = visited_nodes[index]

    # on met a jour la liste des noeuds visités
    visited_nodes.append(graphe)
    visited_nodes_values.append(graphe.value)
