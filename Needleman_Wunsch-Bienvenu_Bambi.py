...
def calcul_alignements(seq1, seq2, match, mismatch, gap, scores, i=None, j=None, graphe=None):
    ...
    if (i==0 and j==0):
        #graphe.diagonale = Tree((i,j))
        return

    if i == 0 and j != 0:
        graphe.haut = Tree((i,j))
        calcul_alignements(seq1, seq2, match, mismatch, gap, scores, i, j-1, graphe.haut)

    if j == 0 and i != 0:
        graphe.gauche = Tree((i,j))
        calcul_alignements(seq1, seq2, match, mismatch, gap, scores, i-1, j, graphe.gauche)
...
"""
Si i et j augmentent ensemble, aligner le nucleotide i avec j (apres augmentation)
Si j augmente et i reste fixe, aligner le nucleotide j avec un gap (apres augmentation)
Si i augmente et j reste fixe, aligner le nucleotide i avec un gap (apres augmentation)
"""
